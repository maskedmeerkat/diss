...
def limitCertainty(m, uMin):
    if (m[2] < uMin):
        m[:-1] = (1-uMin)*m[:-1]
        m[2] = 1 - m[0] - m[1]
    return m

def squishUnRange(m,u_min):
    du = m[2]*(1-u_min)
    m[:-1] += du/2
    m[2] = 1 - m[0] - m[1]
    return m
...
def fuseMasses(m1, m2, combRule = 0, uMin=0.0, entropyScaling=False, eps=1e-4,convergenceAmpFactor=1.):
    '''
    ...
    if (entropyScaling):
        ...
        m = squishUnRange(m1,uMin)
        
        # du = np.max((0,uMin - m1[2]))
        # # obtain the mass not associated with unknown
        # notUnMass = np.sum(m1[:-1])
        # # rescale the free and occ masses 
        # if (notUnMass>0):
        ...
        h1_2 = np.max((0,convergenceAmpFactor*(m2[2] - m1[2])+k))
        # limit the difference in information
        h1_2_max = (m2[2] + k - uMin) / np.max((m2[2] * (1 - m1[2]),eps))
    ...
    # # limit certainty
    # if (entropyScaling):
    #     du = np.max((0,uMin - m[2]))
    #     # obtain the mass not associated with unknown
    #     notUnMass = np.sum(m[:-1])
    #     # rescale the free and occ masses 
    #     if (notUnMass>0):
    #         m[:-1] -= du * m[:-1]/notUnMass
    #         m[2] = 1. - np.sum(m[:-1])
...
mMeas[0,2*numMeasPart:3*numMeasPart] = 0.8
mMeas[1,2*numMeasPart:3*numMeasPart] = 0.0
mMeas[2,2*numMeasPart:3*numMeasPart] = 0.2
...
for it in range(numMeas):        
    ...
    if (it==0):
        ...
        mUnDiff[:,[it]] = fuseMasses(mMeas[:,[it]].copy(), mUnDiff[:,it-1] ,uMin=uMin, convergenceAmpFactor=ampFactor,entropyScaling=True, combRule=0)
