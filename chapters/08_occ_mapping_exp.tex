...
\begin{center}
	...
	\captionof{figure}{\label{fig:comparison_of_maps_diff_sens}Qualitative results of the occupancy maps created using different \gls{ism}s. The abbreviations are explained in Section \ref{subsec:setup_analyze_deep_ism_maps_diff_sensors}.}
\end{center}
...
\begin{center}
	...
	\end{tabular}
	\captionof{figure}{\label{tab:prior_violations}Analysis of the amount of lower bound violations. A violation is given when the maps unknown mass $m_u$ falls below the lower bound $\underline{m}_u=0.3$ for pixels untouched by the geo \gls{irm} (geo \gls{irm} map's unknown mass equals one). The violations are computed for the strict lower bound and given some slack of $0.002$ on the lower bound.}
\end{center} 
