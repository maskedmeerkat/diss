...
\begin{center}
	...
	\captionof{figure}{\label{fig:ds_comb_rule_comparison}This figure shows the qualitative evaluation of the different evidential combination rules given input signals for free, occupied and unknown mass (first subplot) over time with an exemplary lower unknown threshold $\underline{m}_u$ overlayed.}
\end{center}
...
\begin{center}
	\begin{tabular}{c|c|ccc|ccc}
		...
		Yager &$p(k|o)$ & \textcolor{myred}{16.1} & \textcolor{mygreen}{36.3} & 47.6& \textcolor{myred}{15.9} & \textcolor{mygreen}{36.0} & 48.1 \\
		...
		Yager &$p(k|o)$ & \textcolor{myred}{27.2} & \textcolor{mygreen}{69.6} & 3.2& \textcolor{myred}{27.3} & \textcolor{mygreen}{69.4} & 3.3 \\
		...
		Yager &$p(k|o)$ & \textcolor{myred}{27.3} & \textcolor{mygreen}{69.5} & 3.2& \textcolor{myred}{27.4} & \textcolor{mygreen}{69.3} & 3.3 \\
		&$p(k|u)$ & 29.0 & 57.4 & 13.6& 27.3 & 64.4 & 8.3 \\
		& mIoU & - & - & - &15.3&12.4&2.7 \\
		\hline
		deep IR$_{20}$M &$p(k|f)$ & \textcolor{mygreen}{87.4} & \textcolor{myred}{10.5} & 2.1& \textcolor{mygreen}{69.4} & \textcolor{myred}{26.7} & 3.9 \\
		Yager + cutoff&$p(k|o)$ & \textcolor{myred}{28.3} & \textcolor{mygreen}{67.0} & 4.7& \textcolor{myred}{28.4} & \textcolor{mygreen}{66.8} & 4.8 \\
		&$p(k|u)$ & 33.7 & 34.7 & 31.6& 30.4 & 51.9 & 17.7 \\
		& mIoU & - & - & - &15.1&16.5&5.2 \\			
		\hline
		deep, red. IR$_{20}$M &$p(k|f)$ & \textcolor{mygreen}{88.5} & \textcolor{myred}{5.9} & 5.6& \textcolor{mygreen}{71.6} & \textcolor{myred}{16.7} & 11.7 \\
		Yager &$p(k|o)$ & \textcolor{myred}{29.0} & \textcolor{mygreen}{55.7} & 15.3& \textcolor{myred}{29.0} & \textcolor{mygreen}{55.4} & 15.6 \\
		&$p(k|u)$ & 26.3 & 12.9 & 60.8& 26.3 & 23.4 & 50.3 \\
		& mIoU & - & - & - &16.8&23.4&11.8 \\
		\hline
		deep, red. \& geo IR$_{20}$M  &$p(k|f)$ & \textcolor{mygreen}{88.9} & \textcolor{myred}{5.8} & 5.3& \textcolor{mygreen}{72.9} & \textcolor{myred}{16.4} & 10.7 \\
		Yager&$p(k|o)$ & \textcolor{myred}{29.7} & \textcolor{mygreen}{56.6} & 13.7& \textcolor{myred}{29.7} & \textcolor{mygreen}{56.3} & 14.0 \\
		&$p(k|u)$ & 26.8 & 14.3 & 58.9& 26.9 & 25.2 & 47.9 \\
...
\begin{center}
	...
	\captionof{figure}{\label{fig:qual_analysis_of_redundant_info}Illustration of qualitative results for the different occupancy mapping variants for two scenes (A \& B).}
\end{center}
...
In this section, RQ\ref{requ:comp_deep_occ_maps} shall be tackled concerning the comparison of occupancy mapping results given the proposed deep \gls{ism} with different inputs. The setup for comparing occupancy maps given different sensor modalities is the same as described in \ref{subsec:setup_of_red_analy}. The only difference is that for this experiment the fusion method is chosen to be the redundancy reduced accumulation with Yager's rule for all variants (abbrev. as "deep, red."). The compared ISMs are all trained ShiftNets while the inputs are \gls{monodepth} (\gls{c_d}), lidar (\gls{l}) and their respective combinations with the accumulated radar point cloud of the last 20 steps (R$_{20}$). 
%==========================================================================%
%
%==========================================================================%
...
\begin{center}
		\begin{tabular}{c|c|ccc|ccc}
			& \backslashbox{}{\scriptsize{$k$}} & $f$ & $o$ & $u$ & $f$ & $o$ & $u$\\
			\hline
			\gls{c_d} &$p(k|f)$ & \textcolor{mygreen}{89.1} & \textcolor{myred}{1.1} & 9.8& \textcolor{mygreen}{72.5} & \textcolor{myred}{3.3} & 24.2 \\
			...
			\hline
			\gls{c_d} \& R$_{20}$ &$p(k|f)$ & \textcolor{mygreen}{88.4} & \textcolor{myred}{2.3} & 9.3& \textcolor{mygreen}{69.2} & \textcolor{myred}{7.2} & 23.6 \\
			...
			& & \multicolumn{3}{c|}{\scriptsize{whole mapped area}} & \multicolumn{3}{c}{\scriptsize{boundary area}}
	\end{tabular}
	\captionof{table}{\label{tab:conf_mat_deep_ism_map_comparison}Normed confusion matrix for deep ISMs based on ShiftNet applied on different sensor modalities. See \ref{subsec:setup_analyze_deep_ism_maps_diff_sensors} for further information on the methods and abbreviations used.}
\end{center}
...
\begin{center}
	...
	\captionof{figure}{\label{fig:comparison_of_maps_diff_sens}Qualitative results of the occupancy maps created using different ISMs. The abbreviations are explained in Section \ref{subsec:setup_analyze_deep_ism_maps_diff_sensors}.}
\end{center}
...
\begin{center}
	\begin{tabular}{c|ccc}
		...
		\hline
		\scriptsize{fused Map(m$_u$ < 0.300) | geo \gls{irm} Map(m$_u$ = 1), $\underline{m}_u$=0.3} & \scriptsize{0.00023\%} & \scriptsize{2476}\\
		\scriptsize{fused Map(m$_u$ < 0.298) | geo \gls{irm} Map(m$_u$ = 1), $\underline{m}_u$=0.3} & \scriptsize{0.0\%} & \scriptsize{0}
	\end{tabular}
	\captionof{figure}{\label{tab:prior_violations}Analysis of how often the lower bound on the unknown mass $\underline{m}_u$ has been violated in the fused map in areas where the geo \gls{irm} has not been active. The violations are given in percentage and absolute. Also, the amount of violations given some slack on the condition are provided.}
\end{center}
...
\hline
	\scriptsize{geo \gls{irm}} &$p(k|f)$ & \textcolor{mygreen}{59.9} & \textcolor{myred}{8.1} & 32.0& \textcolor{mygreen}{100.0} & \textcolor{myred}{0.0} & 0.0& \textcolor{mygreen}{0.0} & \textcolor{myred}{20.9} & 79.1 \\
...
\hline
	\scriptsize{deep \& geo \gls{irm}} &$p(k|f)$ & \textcolor{mygreen}{78.3} & \textcolor{myred}{16.7} & 5.0& \textcolor{mygreen}{88.1} & \textcolor{myred}{9.3} & 2.6& \textcolor{mygreen}{63.1} & \textcolor{myred}{28.4} & 8.5 \\
...
\hline
	\scriptsize{deep \& geo \gls{irm}} &$p(k|f)$ & \textcolor{mygreen}{77.8} & \textcolor{myred}{13.0} & 9.2& \textcolor{mygreen}{91.4} & \textcolor{myred}{6.0} & 2.6& \textcolor{mygreen}{57.0} & \textcolor{myred}{24.1} & 18.9 \\
