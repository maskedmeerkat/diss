...
\\
\begin{requ}[RQ\ref{requ:filter_outliers_in_geo_irm}] \label{requ:filter_outliers_in_geo_irm}
...
\begin{algorithm}[H]
	...
	\For{$(\mu_r, \mu_\varphi)$ \text{in} currentDetections}{
		...
		\FuncCall{castFreeSpaceRay}{$\varphi_{\text{center}}$, $r_{\text{det}}$, $\varphi_\sphericalangle$, $M_F$}\;		
		\FuncCall{DempsterRule}{\FuncCall{castFreeSpaceRay}{$\varphi_{\text{center}}$, $r_{\text{det}}$, $\varphi_\sphericalangle^{(2)}$, $M_F^{(2)}$}}\;
		\If{(detection is static)}{
