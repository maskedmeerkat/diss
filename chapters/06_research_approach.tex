...
\begin{align}
	...
	I_{T|0:T} &\approx \Delta m_{u} + K \in [-1,1]
\end{align}
In case a new measurement contains conflicting information $K$ but is also less certain than the current map state, $Delta m_u$ will become negative and reduce the amount of novelty in the update. This effect is desired to reduce the influence of outliers. The qualitative evaluation in the bottom plot of Fig. \ref{fig:ds_comb_rule_comparison} shows the approximated amount of novel information in several fusion constellations.

At this point, it should be mentioned that the map region around the vehicle could simply be updated by replacing the old map state with the new deep \gls{ism} estimate. This, however, would remove all of the geometric \gls{ism}'s influences. Therefore, it is rather proposed to discount the deep \gls{ism} estimate according to the amount of non-redundant information and combine it with the current map state.\\
...
\begin{align}
	...
	\begin{cases}
		0, I_{T|0:T} < 0\\
		0, \lim\limits_{I_{T|0:T} \rightarrow 0}\\
		1, \lim\limits_{I_{T|0:T} \rightarrow 1}\\
	\end{cases}
\end{align}
It should be noted that, ideally, $I_{T|0:T} < 0$ never occurs since the next time step's estimate is based on at least the same amount of information as the previous one. Since the proposed approximation of $I_{T|0:T}$ in \eqref{eq:non_redund_info} lies within the interval $[-1,1]$, the behavior defined in Eq. \ref{eq:discount_factor_behavior} can e.g. be achieved in a linear way as follows
\begin{align}
