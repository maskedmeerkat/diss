...
%
%======================================%
\subsection{Requirements for deep ISMs}
...
%
%======================================%
\subsection{Requirements for Usage of deep ISMs as Priors in Occupancy Mapping}
...
%
%======================================%
\subsection{Research Needs for deep, evidential ISMs}
...
\begin{center}
	...
	\captionof{figure}{\label{fig:illustrative_system_overview}Structural overview of the proposed framework, showing how the \gls{bev} input is transformed both by the deep and geo \gls{ism} into occupancy estimates.}
\end{center}
...
\begin{algorithm}[H]
	...
	\textbf{Hyperparameters:} $\gls{sym:idm_mf}, \gls{sym:idm_mo}, \gls{sym:idm_md}, \gls{sym:open_angle}$\;
	\Function{castFreeSpace}{
		\Comment{see \eqref{eq:idm_radar} with $\gls{sym:idm_mo} = \gls{sym:idm_md} = 0$}
	}
	remove ground-plane detections\;
	initialize all cells in the \gls{ilm} image to $\gls{sym:m_u}=1$\;
	\For{$\gls{sym:center_angle}$ \text{in} discretizedAngles360}{
		\If{(at least one detection in current cone area)}{
			$(\gls{sym:r_det}, \gls{sym:phi_det})$ = \FuncCall{getClosestDetectionInsideConeArea}{$\gls{sym:center_angle}$, $\gls{sym:open_angle}$, $\gls{sym:r_max}$}\;
			\FuncCall{castFreeSpaceRay}{$\gls{sym:center_angle}$, $\gls{sym:r_det}$, $\gls{sym:open_angle}$, $\gls{sym:idm_mf}$}\;
			\Comment{mark closest detection}
			\If{(detection is static)}{
				\FuncCall{assignCellValue}{$(\gls{sym:r_det}, \gls{sym:phi_det})$, $[0,\gls{sym:idm_mo},1-\gls{sym:idm_mo}]$}\;
			}
			\Else{
				\FuncCall{assignCellValue}{$(\gls{sym:r_det}, \gls{sym:phi_det})$, $[\gls{sym:idm_md}, \gls{sym:idm_md}, 1-2\gls{sym:idm_md}]$}\;
			}
		}
		\Else{
			\FuncCall{castFreeSpaceRay}{$\gls{sym:center_angle}$, $\gls{sym:r_max}$, $\gls{sym:open_angle}$, $\gls{sym:idm_mf}$}\;			
		}
	}
\end{algorithm}
%======================================%
...
\begin{algorithm}[H]
	...
	\textbf{Hyperparameters:} $\gls{sym:idm_mf}^{(\text{\scriptsize thin})}, \gls{sym:idm_mf}^{(\text{\scriptsize big})}, \gls{sym:open_angle}^{(\text{\scriptsize thin})}, \gls{sym:open_angle}^{(\text{\scriptsize big})}, \gls{sym:idm_mo}, \gls{sym:idm_md}, \gls{sym:temp_hor}$\;	
	\Function{castFreeSpace}{
		\Comment{see \eqref{eq:idm_radar} with $\gls{sym:idm_mo} = \gls{sym:idm_md} = 0$}
	}
	initialize all cells in the \gls{irm} image to $m_u=1$\;
	transform previous \gls{sym:temp_hor} radar point clouds of all radars to current \gls{irm} image\;
	\For{$(\gls{sym:r_det}, \gls{sym:phi_det})$ \text{in} currentDetections}{
		\Comment{range at which the free space of the ray is stopped}
		$(\gls{sym:r_det}, \gls{sym:phi_det})$ = \FuncCall{getClosestDetectionInsideConeArea}{$\gls{sym:center_angle}$, $\gls{sym:open_angle}$, $\gls{sym:r_max}$}\;	
		\Comment{cast thin and big free space rays}
		\FuncCall{castFreeSpaceRay}{$\gls{sym:center_angle}$, $r_{\text{det}}$, $\gls{sym:open_angle}^{(\text{\scriptsize thin})}$, $\gls{sym:idm_mf}^{(\text{\scriptsize thin})}$}\;		
		\FuncCall{DempsterRule}{\FuncCall{castFreeSpaceRay}{$\gls{sym:center_angle}$, $r_{\text{det}}$, $\gls{sym:open_angle}^{(\text{\scriptsize big})}$, $\gls{sym:idm_mf}^{(\text{\scriptsize big})}$}}\;
		\If{(detection is static)}{
			\FuncCall{assignCellValue}{$(\gls{sym:r_det}, \gls{sym:phi_det})$, $[0,\gls{sym:idm_mo},1-\gls{sym:idm_mo}]$}\;
		}
		\Else{
...
\end{align}
Since the above derivation is made for the case of combination in the initialization phase, both the map cell's and deep \gls{ism} estimate's unknown masses are in the interval $\gls{sym:m_u}^{0:\gls{sym:temp_hor}-1},\quad \tilde{m}_u^{0:\gls{sym:temp_hor}}\in[\gls{sym:m_u_},1]$. Thus, for $\zeta\equiv 0$ \eqref{eq:lower_bound_condition} is satisfied.\\
