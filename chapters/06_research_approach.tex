...
\begin{algorithm}
	...
	\For{angle \text{in} discretizedAnlges360}{
		\If{(no detection along current angle)}{
			\FuncCall{castFreeSpaceRay}{angle, maxRange, openingAngle=angle, pF}\;
		}
		\Else{
...
\end{align}
Once the unknown mass has fallen below the threshold $\underline{m}_u$, the \textbf{convergence phase} starts. Here, the geometric \gls{ism} should still be integrated into the map while the influence of the deep \gls{ism} should be disabled, guaranteeing a convergence towards the geometric \gls{ism}. To achieve this, a combination rule has to be chosen that integrates geometric \gls{ism} estimates in a way to strictly reduce the unknown mass but is also capable to shift mass between the free and occupied class to e.g. account for changes in the occupancy state (parked car starts to move) or for correction purposes.
The three phases of the proposed procedure are depicted in Fig. \ref{fig:three_phased_approach}. In the next section, the choices for combination rules for both the deep and geometric \gls{ism} to suffice the requirements of each phase are elaborated on. Afterwards, in sec. \ref{subsec:adaptation_of_redund_removal_to_sat_m_u_lower}, an adaptation of the discount factor to remove the redundancy in a way to respect the lower bound on the unknown mass $\underline{m}_{u}$ is defined.
\begin{figure}
	\begin{center}
...
%
%======================================%
\subsection{Choice of Combination Rule for Initialization and Convergence Phase}
\label{subsec:choice_of_combination_rule_for_init_and_converge_phase}
The first step to obtain the behavior as described in sec. \ref{subsec:method_to_use_deep_isms_in_occmaps}, is an analysis of the requirements for both the initialization and convergence phase. 
Starting with the initialization phase, 
%======================================%
%
%======================================%
\subsection{Adaptation of Redundancy Removal to satisfy lower Bound on unknown Mass}
\label{subsec:adaptation_of_redund_removal_to_sat_m_u_lower}
