...
\begin{align}
	...
	\end{cases}
\end{align}
A visualization of the ideal \gls{idm} is depicted in Fig. \ref{fig:idm} a). Next, since the radial measurement $z = (\tilde{\mu}_r, \tilde{\mu}_{\phi})$ is in fact not ideal but contains uncertainty, the ideal \gls{idm} is convoluted with a radial and angular Gaussian noise model. Here, Elfes argues that this model can only be evaluated in closed form for special sensor models and, thus, instead provides visualizations of the numerical results.
\\\\
In \cite{loop2016closed}, Loop et al. provide a formulation of the \gls{idm}'s radial component along $\mu_\phi$ using the error function $\text{Erf}(x) = \dfrac{2}{\pi} \int_{0}^{x}e^{-t^2}dt$ and a small object thickness $\tau$ as follows 
\begin{align}
	\label{eq:gauss_idm}
	\text{\gls{idm}}_{\text{Gauss}}(r,\tilde{\mu}_r, \phi = \mu_\phi) = P(p_o(r, \phi = \mu_\phi) = 1 |\tilde{\mu}_r)  =& \dfrac{1}{2}\text{Erf}\left(\dfrac{\tilde{\mu}_r-r}{\sqrt{2}\sigma_r}\right)\\
	&- \dfrac{1}{4}\text{Erf}\left(\dfrac{\tilde{\mu}_r - r - \tau}{\sqrt{2}\sigma_r}\right) + \dfrac{1}{4} \notag
\end{align}
Given that formulation, they continue to show that the application of a Gaussian noise model always leads to a decision boundary between free and occupied space slightly in front of the actual object boundary. This effect can already be seen in Fig. 3 of Elfes' publication. They, hence, propose a quadratic b-spline as an alternative noise model to tackle the problem. Their final radial \gls{idm} after convolving the proposed b-spline with their ideal \gls{idm} can be written as follows
\begin{align}
	\label{eq:bspline}
	\text{BSpline}(x) &= 
	\begin{cases}
...
%
	...
	\text{\gls{idm}}_{\text{B-Spline}}(r,\tilde{\mu}_r, \phi = \mu_\phi) &= \text{BSpline}((r - \tilde{\mu}_r)/\sigma_r) - \dfrac{1}{2}\text{BSpline}((r - \tilde{\mu}_r)/\sigma_r - 3)
\end{align}
The application of the b-spline noise model onto the ideal \gls{idm}, which has been widely adopted in evidential occupancy mapping \cite{mouhagir2017using,reineking2013evidential,yu2015evidential}, is depicted in Fig. \ref{fig:idm} b), c).
\begin{figure}
	\begin{center}
...
\end{align}
Eventually, to arrive at the \gls{ism}, this \gls{idm} is applied on each detection of the sensor measurement and their influences are accumulated using one of the fusion approaches described in \ref{subsec:combination_of_independent_evidence}.
%======================================%
%
%======================================%
...
%		\caption{\label{fig:conv_layers} \gls{conv} layer}
%	\end{center}
%\end{figure}  
%\begin{center}
%	\begin{tabular}{c|c|c}
